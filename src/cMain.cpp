...
cMain::cMain()
    : wxFrame(nullptr, wxID_ANY, "Checkers App", wxPoint(0, 19),
              wxSize(800, 800),
              wxDEFAULT_FRAME_STYLE &
                  ~(wxRESIZE_BORDER | wxMAXIMIZE_BOX)) // & wxFRAME_NO_TASKBAR
{
  ...
  for (int col = 0; col < 8; col++) {
    for (int row = 0; row < 8; row++) {
      ...
      if (row < 3 && ((row % 2 == 0 && (row * 8 + col) % 2 == 1) ||
                      (row % 2 == 1 && (row * 8 + col) % 2 == 0))) {
        tile = new BoardTile(game, this, 1000 + (row * 8 + col),
                             wxPoint(col * 120, row * 120), wxSize(120, 120));
        tile->setRow(row);
        tile->setCol(col);
        ...
        game->addToken(token);
        board->addTile(tile);
      } else if (row >= 5 && ((row % 2 == 0 && (row * 8 + col) % 2 == 1) ||
                              (row % 2 == 1 && (row * 8 + col) % 2 == 0))) {
        tile = new BoardTile(game, this, 1000 + (row * 8 + col),
                             wxPoint(col * 120, row * 120), wxSize(120, 120));
        tile->setRow(row);
      ...
      if ((row % 2 == 0 && (row * 8 + col) % 2 == 0) ||
          (row % 2 == 1 && (row * 8 + col) % 2 == 1)) {
