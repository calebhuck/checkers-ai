...
PlayerType Game::getCurrentPlayer() { return current_player; }
...
Tokens *Game::getAITokens() { return ai_tokens; }
Tokens *Game::getPlayerTokens() { return player_tokens; }
...
void Game::reportClick(BoardTile *tile) {
  if (current_player == PLAYER) {
    // no tiles are selected and the current player has a piece on the selected
    // tile
    if (!is_a_tile_selected && tile->isTokenPresent() &&
        tile->getToken()->getPlayer() == PLAYER) {
      tile->setSelected();
      tile->Refresh();
      tile->Update();
    ...
    else if (is_a_tile_selected && !tile->isTokenPresent() &&
             selected != tile) {
      if (validateMove(selected->getRow(), selected->getCol(), tile->getRow(),
                       tile->getCol())) {
        // LOG("move player")
        move(selected, tile);
        check_end_game();
...
bool Game::validateMove(int from_row, int from_col, int to_row, int to_col) {
  // if to or from location is out of bounds, return false right away and print
  // error message
  if (from_row < 0 || from_row > 7) {
    std::cout << "Invalid from location send to validateMove...\n";
    return false;
  }
  ...
  if (to_row < 0 || to_row > 7) {
    std::cout << "Invalid to location sent to validateMove...\n";
    return false;
  }
  if (to_col < 0 || to_col > 7) {
    std::cout << "Invalid to location send to validateMove...\n";
    return false;
  }
  ...
  if (token->getPlayer() == PLAYER) {
    ...
    // king token
             ...
             !getTile(to_row, to_col)->isTokenPresent() && token->isKing()) {
      // get piece to be jumped to determine if it is an enemy piece
      Token *temp;
      if (to_col > from_col)
        temp = getTile(to_row - 1, to_col - 1)->getToken();
...
bool Game::move(BoardTile *from, BoardTile *to) {
  ...
  if (validateMove(from_row, from_col, to_row, to_col)) {
      ...
      if (to_col > from_col) {
        ...
        if (token->getPlayer() == PLAYER) {
          ...
          if (to_row < from_row) {
            ...
            if (jumped->isTokenPresent() &&
                jumped->getToken()->getPlayer() == AI) {
              ai_tokens->remove(jumped->getToken());
              jumped->removeToken();
            } else {
              std::cout << "ERROR: Invalid move made, validateMove function "
                           "should not allow this...\n";
              // throw -1;
            }
          }
          // case where jump is made backward, the token must be a king to do
          // this
        ...
        // jump was made by the AI
          ...
          if (to_row > from_row) {
            jumped = getTile(to_row - 1, to_col - 1);
            if (jumped->isTokenPresent() &&
                jumped->getToken()->getPlayer() == PLAYER) {
    ...
    // check for additional jumps, all available jumps must be made before the
    // turn ends
    if (jump_move) {
      int row = to_row;
      int col = to_col;
      ...
      if (token->getPlayer() == PLAYER) {
        if (row - 2 >= 0) {
          ...
          if (col - 2 >= 0) {
            if (!getTile(row - 2, col - 2)->isTokenPresent() &&
                getTile(row - 1, col - 1)->isTokenPresent()) {
      ...
      if (token->getPlayer() == AI) {
        ...
        if (row + 2 <= 7) {
          ...
          if (col - 2 >= 0) {
            if (!getTile(row + 2, col - 2)->isTokenPresent() &&
                getTile(row + 1, col - 1)->isTokenPresent()) {
              if (getTile(row + 1, col - 1)->getToken()->getPlayer() ==
                  PLAYER) {
...
BoardTile *Game::getTile(int row, int col) { return board->getTile(row, col); }
