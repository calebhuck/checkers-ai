...
void MiniMaxAI::get_move() {
  // create the array of AIMove structures so each thread can return best move
  ...
  // determine how many threads to make (equal to the number of ai tokens on the
  // board) and start the threads
  ...
  while (count < num_threads) {
    ...
    if (temp != nullptr) {
      ...
      // create worker to evaluate possible moves of this token with its own
      // copy of board
      workers[count] = std::thread(&MiniMaxAI::findMove, this, temp->getID(),
                                   DEPTH, AI, board, moves);
  ...
  // outfile.open(std::to_string(1) + "_thread_" + std::to_string(DEPTH) +
  // "_depth.txt", std::ios_base::app); outfile << std::fixed <<
  // std::setprecision(10) << diff.count() << std::endl; std::cout <<
  // diff.count() << std::endl;
  ...
  if (best_move != nullptr) {
    if (game->getTile(best_move->from_row, best_move->from_col) != nullptr &&
        game->getTile(best_move->to_row, best_move->to_col) != nullptr) {
...
int MiniMaxAI::findMove(int id, int depth, PlayerType turn, Token **board,
                        AIMove **moves) {
  if (depth == DEPTH) {
    ...
    // check each possible move for this particular token and search the
    // resulting tree for moves recursively
    ...
    if (validateMove(board, row, col, row + 2, col + 2)) {
      ...
      if (score >= best_score) {
        ...
        best_score = ++score;
        temp_board = getBoardCopy(board);
        move(temp_board, ai_move->from_row, ai_move->from_col, ai_move->to_row,
             ai_move->to_col);
        if (validateMove(temp_board, ai_move->to_row, ai_move->to_col,
                         ai_move->to_row + 2, ai_move->to_col + 2)) {
          best_score += 100;
          ai_move->score += 100;
        }
        if (validateMove(temp_board, ai_move->to_row, ai_move->to_col,
    ...
    if (token->isKing()) {
      ...
      if (validateMove(board, row, col, row - 2, col + 2)) {
        ...
        if (score >= best_score) {
          ...
          best_score = ++score;
          temp_board = getBoardCopy(board);
          move(temp_board, ai_move->from_row, ai_move->from_col,
        ...
        if (token->getPlayer() == AI) {
          ...
          if (validateMove(board, row, col, row + 2, col + 2)) {
            ...
            if (score >= best_score) {
              ...
              if (validateMove(temp_board, new_row, new_col, new_row + 2,
                               new_col + 2)) {
                best_score += 100;
              }
              if (validateMove(temp_board, new_row, new_col, new_row + 2,
...
bool MiniMaxAI::validateMove(Token **board, int from_row, int from_col,
  ...
  if (token->getPlayer() == PLAYER) {
    ...
    if (to_row - from_row == -1 && (abs(to_col - from_col) == 1) &&
        board[to_row * 8 + to_col] == nullptr)
      return true;
    // player token moves 1 space backward (must be a king)
    else if (to_row - from_row == 1 && (abs(to_col - from_col) == 1) &&
             board[to_row * 8 + to_col] == nullptr && is_king)
      return true;
    ...
    // player token moves two spaces backward, must jump an enemy piece and be a
    // king token
    else if (to_row - from_row == 2 && (abs(to_col - from_col) == 2) &&
             board[to_row * 8 + to_col] == nullptr && token->isKing()) {
      // get piece to be jumped to determine if it is an enemy piece
      Token *temp;
      if (to_col > from_col)
        temp = board[(to_row - 1) * 8 + (to_col - 1)];
  ...
  if (token->getPlayer() == AI) {
    ...
    // ai token moves two spaces backward, must jump an enemy piece and be a
    // king token
    if (to_row - from_row == -2 && (abs(to_col - from_col) == 2) &&
        board[to_row * 8 + to_col] == nullptr && token->isKing()) {
      // get piece to be jumped to determine if it is an enemy piece
...
void MiniMaxAI::move(Token **board, int from_row, int from_col, int to_row,
                     int to_col) {
  Token *token = board[from_row * 8 + from_col];
  token->setRow(to_row);
  token->setCol(to_col);
      ...
      if (token->getPlayer() == PLAYER) {
        ...
        if (to_row < from_row) {
          ...
          if (jumped != nullptr) {
            ...
            std::cout << "ERROR1: Invalid move made, validateMove function "
        ...
        // case where jump is made backward, the token must be a king to do this
          ...
          if (jumped != nullptr) {
            ...
            std::cout << "ERROR2: Invalid move made, validateMove function "
                         "should not allow this...\n";
            // throw -1;
          }
        }
      ...
      // jump was made by the AI
        ...
        if (to_row > from_row) {
          ...
          if (jumped != nullptr) {
            ...
            std::cout << "ERROR3: Invalid move made,  validateMove function "
                         "should not allow this...\n";
            // throw -1;
          }
        }
        // case where jump is made backward, the token must be a king to do this
          ...
          if (jumped != nullptr) {
            ...
            std::cout << "ERROR4: Invalid move made, validateMove function "
                         "should not allow this...\n";
            // throw -1;
          }
        }
      ...
      if (token->getPlayer() == PLAYER) {
        ...
        if (to_row < from_row) {
          ...
          if (jumped != nullptr) {
            ...
            std::cout << "ERROR5: Invalid move made, validateMove function "
                         "should not allow this...\n";
            // throw -1;
          }
        }
        // case where jump is made backward, the token must be a king to do this
          ...
          if (jumped != nullptr) {
            ...
            std::cout << "ERROR6: Invalid move made, validateMove function "
                         "should not allow this...\n";
            // throw -1;
          }
        }
      ...
      // jump was made by the AI
        ...
        if (to_row > from_row) {
          ...
          if (jumped != nullptr) {
            ...
            std::cout << "ERROR7: Invalid move made,  validateMove function "
                         "should not allow this...\n";
            // throw -1;
          }
        }
        // case where jump is made backward, the token must be a king to do this
          ...
          if (jumped != nullptr && jumped->getPlayer() == PLAYER) {
            ...
            std::cout << "ERROR8: Invalid move made, validateMove function "
                         "should not allow this...\n";
            // throw -1;
          }
        }
